...
# prediction phenotype values for all samples (only one phenotype predicted at a time)
#
...
if(length(args) != 3)
{
    cat("usage: predict_phenotypes.R <genotype_file> <model_file> <output_file>\n\n")
...
#modelfile <- "uniqsplit_glmnetcvalpha_score_glmnet_cvalpha.csv"
#genofile <- "uniq_genotypes_numerical_split.csv"
#outfile <- "test_predict.csv"
...
for(col in colnames(comb)[4:ncol(comb)])
{
    comb[col] <- comb$value * comb[col]
}

#sum and add baseline
pred <- as.data.frame(colSums(comb[4:ncol(comb)])+baseline)
